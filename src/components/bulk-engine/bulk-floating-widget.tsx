...
/**
 * AETHER BULK ENGINE — floating progress widget.
 *
 ...
 *
 * Minimized state shows ONLY: status icon, title, progress bar, batch count,
 * percentage, and elapsed time. No ETA text (ETA is shown only in the full
 * dialog for precision; the widget stays compact and glanceable).
 *
...
export function BulkFloatingWidget() {
  ...
  // updates live (Dexie useLiveQuery only re-renders on data changes,
  ...
  const accentBar = isProcessing ? 'bg-emerald-500' : isPaused ? 'bg-amber-500' : isPartial ? 'bg-amber-500' : isFailed ? 'bg-red-500' : 'bg-emerald-500'
        ...
        >
          <div className="relative w-[260px] max-w-[calc(100vw-1.5rem)]">
            <button
              ...
              className="group relative w-full overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0B1220]/95 backdrop-blur-md shadow-2xl shadow-black/40 text-left transition-all hover:border-white/20"
            >
