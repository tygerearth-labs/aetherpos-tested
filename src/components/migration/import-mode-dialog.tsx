...
const modes: {
  ...
  },
  {
    ...
    recommended: true,
    description: 'Import produk sekaligus stok gudang. Setiap produk (dan tiap varian) otomatis dibuatkan InventoryItem + terhubung 1:1 secara atomic — cocok untuk bisnis ritel di mana stok = produk yang dijual. Stok=0 tetap ter-link.',
    features: [
      { icon: Package, label: 'Produk' },
      { icon: Warehouse, label: 'Stok Gudang' },
      { icon: Link2, label: 'Auto-link 1:1 (atomic)' },
      { icon: BarChart3, label: 'Stok Awal & HPP' },
      { icon: FileText, label: 'SKU & Barcode' },
    ],
    suitedFor: ['Retail', 'Minimarket', 'Elektronik', 'Farmasi', 'Fashion reseller', 'Beauty', 'F&B tanpa resep'],
    note: 'Product + InventoryItem + ProductComposition 1:1 + hasComposition=true dibuat atomic. Stok produk berkurang saat terjual. Varian punya InventoryItem sendiri. Konflik dengan BOM manual → skip eksplisit.',
  },
  {
    ...
    subtitle: 'Resep / BOM',
    recommended: false,
    description: 'Import produk, bahan baku terpisah, dan resep/komposisi (BOM). Untuk bisnis yang mengolah bahan menjadi produk jadi. Tidak ada auto-create InventoryItem dari baris produk — link hanya dari inline komposisi atau Sheet BOM.',
    features: [
      { icon: Package, label: 'Produk' },
      ...
      { icon: Lightbulb, label: 'Warning Estimasi Kapasitas' },
    ],
    suitedFor: ['F&B dengan resep', 'Percetakan', 'Konveksi', 'Bangunan', 'Pertanian', 'Manufactur'],
    note: 'InventoryItem hanya dari Sheet Bahan Baku. Link produk↔inventory hanya dari inline komposisi atau Sheet BOM. Product.stock tidak di-clamp; warning estimasi kapasitas muncul jika stok > estimasi BOM (bahan bisa dipakai bersama).',
  },
]
