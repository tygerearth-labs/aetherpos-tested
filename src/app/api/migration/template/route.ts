...
function getGuideData(mode: TemplateMode): (string)[][] {
  ...
  if (mode === 'product_stock') {
    lines.push(
      ...
      ['Setiap produk OTOMATIS dibuatkan InventoryItem + terhubung 1:1.',
       'Hubungan ini WAJIB utuh (atomic): Product + InventoryItem +',
       'ProductComposition(qty=1) + hasComposition=true dibuat bersamaan.',
       'Jika salah satu gagal → seluruh batch rollback (tidak ada orphan).'],
      ...
      ['      • Isi semua produk tanpa varian (ukuran/warna tetap)',
       ...
       '      • STOK AWAL = 0 juga tetap membuat InventoryItem(0) + link 1:1',
       '        (stok gudang mengalir lewat inventory ledger).'],
      [''],
      ['  2️⃣  SHEET "Produk Varian" (jika ada produk dengan ukuran/warna)',
       ...
       '      • Setiap varian punya HPP, harga jual, dan STOK AWAL VARIAN sendiri',
       '      • Tiap varian OTOMATIS dapat InventoryItem sendiri',
       '        (nama: "Nama Produk - Nama Varian") + ProductComposition',
       '        dengan variantId — berbeda dari induk & varian lain.'],
      [''],
      ...
      ['  ✅ Isi kolom STOK AWAL dengan jumlah stok SAAT INI',
       '  ✅ STOK AWAL = 0 tetap membuat InventoryItem(0) + link 1:1 (bukan skip)',
       '  ✅ InventoryItem OTOMATIS dibuat (tidak manual, tidak perlu Sheet Bahan Baku)',
       '  ✅ Setiap produk TERHUBUNG ke inventory (1:1, qty=1)',
       '  ✅ Setiap VARIAN terhubung ke InventoryItem sendiri (qty=1, variantId di-set)',
       '  ✅ Re-migration aman: link 1:1 yang sudah ada tidak di-duplicate',
       '  ✅ Stok otomatis BERKURANG saat terjual di POS; void mengembalikan stok'],
      ['  ❌ Tidak perlu Sheet "Bahan Baku" atau "Komposisi"',
       '  ❌ Tidak perlu resep/bahan mentah (gunakan Mode 3 jika perlu)',
       '  ⚠️ KONFLIK: jika produk/varian sudah punya komposisi manual (BOM)',
       '     di Master Produk, auto-link 1:1 DILEWATI + error eksplisit.',
       '     Hapus komposisi existing untuk mengaktifkan auto-link.'],
      [''],
    )
  }
  ...
  if (mode === 'product_inventory') {
    lines.push(
      ['═══════════════════════════════════════════════════'],
      ['MODE 3: PRODUK + KOMPOSISI (BOM)'],
      ['Untuk Bisnis MANUFAKTUR — Bahan Baku → Produk Jadi via Resep'],
      ['═══════════════════════════════════════════════════'],
      [''],
      ['Mode ini untuk bisnis yang MENGOLAH BAHAN menjadi PRODUK JADI.',
       'Bahan baku terpisah dari produk, terhubung melalui resep/komposisi (BOM).',
       'Stok bahan baku berkurang otomatis saat produk terjual ( sesuai resep).'],
      [''],
      ['KONTRAK MODE 3 (FOUNDER APPROVED):',
       '  • TIDAK ada auto-create InventoryItem dari baris produk.',
       '    Kolom STOK AWAL di Sheet Produk hanya mengisi Product.stock,',
       '    TIDAK membuat InventoryItem dengan nama yang sama.',
       '  • InventoryItem HANYA dibuat dari Sheet "Bahan Baku" (Sheet 3).',
       '  • Link produk↔inventory HANYA dari:',
       '    (a) Sheet "Komposisi (Resep BOM)" — Sheet 4 (multi-bahan, yield), atau',
       '    (b) Kolom KOMPOSISI INLINE di Sheet Produk (resep singkat inline).',
       '  • Jika tidak ada BOM (inline atau Sheet 4) untuk sebuah produk,',
       '    produk itu tidak punya composition → penjualan hanya mengurangi',
       '    Product.stock (Mode-3 degenerate ke Mode-1 untuk produk tsb).',
       '  • Product.stock TIDAK di-clamp ke kapasitas BOM.',
       '  • WARNING estimasi kapasitas: jika Product.stock > estimasi kapasitas BOM,',
       '    migrasi mengeluarkan warning (informational — bahan bisa dipakai bersama).',
       '    Validasi runtime saat penjualan bersifat otoritatif.'],
      [''],
      ['CONTOH BISNIS:',
      ...
      [''],
      ['CARA ISI:',
       '  1. Sheet "Produk Non-Varian" — produk jadi (STOK AWAL opsional,',
       '     KOMPOSISI INLINE opsional untuk resep singkat)',
       '  2. Sheet "Produk Varian" — jika ada varian (KOMPOSISI VARIAN INLINE opsional)',
       '  3. Sheet "Bahan Baku" — DAFTAR SEMUA BAHAN MENTAH Anda',
       '  4. Sheet "Komposisi (Resep BOM)" — resep/bahan per produk (untuk BOM kompleks)',
      ...
      ['PENTING:',
       ...
       '  • STOK AWAL produk TIDAK membuat InventoryItem — hanya Product.stock.',
       '  • WARNING kapasitas BOM muncul jika Product.stock > estimasi kapasitas.',
       '    Estimasi = min over composition rows of (invStock × yieldPerBatch / qty).',
       '    Karena bahan bisa dipakai bersama produk lain, estimasi bisa lebih',
       '    kecil dari kapasitas aktual. Hanya informasi, tidak mengubah data.'],
      [''],
    )
  }
