...
export default function CustomerFormDialog({ open, onOpenChange, customer, onSaved }: CustomerFormDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateWhatsapp(whatsapp)) {
      setError('Nomor WhatsApp harus diawali +62 atau 08')
      return
    }
      ...
      <ResponsiveDialogContent className="bg-nebula border-white/[0.06] p-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 py-1">
          <div className="space-y-1.5">
            <Label className="text-xs text-slate-300">Nama Pelanggan *</Label>
            <Input
              value={name}
          ...
          <div className="space-y-1.5">
            <Label className="text-xs text-slate-300">Nomor WhatsApp *</Label>
            <Input
              value={whatsapp}
              ...
              className="bg-white/[0.04] border-white/[0.08] text-white placeholder:text-slate-500 h-9 text-sm"
            />
            <p className="text-[11px] text-slate-500">Harus diawali +62 atau 08</p>
          </div>
